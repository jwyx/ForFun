...
size_t find_median_of_median(vector<int> &arr, size_t left, size_t len) {
  ...
  size_t group_num = len / SPAN;
  size_t median_index = 0;
  for (size_t i = 0; i < group_num; i ++) {
    insert_sort(arr, left + i * SPAN, SPAN);
    median_index = left + i * SPAN + SPAN / 2;
    medians.push_back(arr[median_index]);
    median_indexes.push_back(median_index);
  }

  size_t left_num = len % SPAN;

  if (left_num) {
    insert_sort(arr, left + group_num * SPAN, left_num);
    median_index = left + group_num * SPAN + left_num / 2;
    medians.push_back(arr[median_index]);
    median_indexes.push_back(median_index);
  }
